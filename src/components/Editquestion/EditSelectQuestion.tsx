...
const EditSelectQuestion = ({ open, setOpen, question }: questionDataProps) => {
  ...
  async function onSubmit(input: UpdateQuestionSchema) {
    ...
    try {
      ...
      if (response.ok) {
        setOpen(false);
        toast.success(e("table.action.edit.suc"));
        router.refresh();
      }
    } catch (error) {
      toast.error(e("table.action.edit.err"));
    }
  }
